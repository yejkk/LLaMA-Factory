...
from transformers import Trainer,GenerationConfig,TrainingArguments
...
# 无论版本如何，总是从_VALID_DICT_FIELDS中移除"model_init_kwargs"
if not is_transformers_version_greater_than("4.51.0"):
    from transformers.training_args import _VALID_DICT_FIELDS
    if "model_init_kwargs" in _VALID_DICT_FIELDS:
        _VALID_DICT_FIELDS.remove("model_init_kwargs")
else:
    _VALID_DICT_FIELDS = [field for field in TrainingArguments._VALID_DICT_FIELDS if field != "model_init_kwargs"]
...
class CustomGRPOTrainer(GRPOTrainer, Trainer):
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if len(self._metrics["loss"]) > 0:
            train_loss = sum(self._metrics["loss"]) / len(self._metrics["loss"])
        ...
        # Average the metrics, but skip empty lists to avoid division by zero
        metrics = {key: sum(val) / len(val) if len(val) > 0 else 0.0 for key, val in self._metrics.items()}
